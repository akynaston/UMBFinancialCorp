...
public final class ConfigLoader {
    static Logger log = LoggerFactory.getLogger(ConfigLoader.class);
    ...
    private static final String XML_ALERTS = "alerts";
    ...
    private static final String XML_MULTI = "multiplier";
    private static final String XML_RETRY = "retry";
    private static final String XML_WAIT = "wait";
    private static final String XML_ALERT_SMTP_SERVER = "smtp-server";
    private static final String XML_ALERT_SENDER = "alert-sender";
    private static final String XML_ALERT_RECIPIENT = "alert-recipient";
    private static final String XML_ALERT_SUBJECT_PREFIX = "subject-prefix";
    private static final String XML_ALERT_LOG_PATH = "log-path";

    private static final String XML_SUBSTITUTIONS = "substitutions";
    private static final String XML_INJECTIONS = "data-injections";
    private static final String XML_REPLACE = "replace";
    private static final String XML_NEW = "new";
    private static final String XML_KEY = "key";
    private static final String XML_FORMAT = "format";
    private static final String XML_MUTATOR = "mutator";
    private static final String XML_FREQUENCY = "frequency";
    private static final String XML_LIVE_PROFILE = "live-profile";
    private static final String XML_ENABLE_EMAIL_ALERTS = "enable-email-alerts";
    private static final String XML_ENABLE_LOG_ALERTS = "enable-log-alerts";
    private static final String XML_ATTRIBUTE_ENCRYPTED = "encrypted";
    private static final String STR_ALERT_PREFIX = "idmunitAlert_";
    ...
    private static Document loadXMLFromFS(String fullPath) throws IdMUnitException {
        ...
        try {
            ...
            doc = parser.build(new File(fullPath));
     ...
     */
    private static Map<String, String> getSubstitutions(Element connection, String encryptionKey) throws IdMUnitException {
        String replaceVal;
        String newVal;
        ...
        for (int i = 0; i < substitutions.size(); ++i) {
            ...
            newVal = getDecryptedChildText(substitution, XML_NEW, encryptionKey);
            if (replaceVal != null && replaceVal.length() > 0 && newVal != null && newVal.length() > 0) {
    ...
    private static List<InjectionConfigData> getDataInjections(Element connection) throws IdMUnitException {
        ...
        for (int i = 0; i < injections.size(); ++i) {
            ...
            String frequency = getChildText(injection, XML_FREQUENCY);

            if (className != null && className.length() > 0 && keyName != null && keyName.length() > 0) {
                log("###\t\tClass used to inject data", className);
                log("###\t\tKey value to replace", keyName);
                log("###\t\tFormat of injected data", format);
                log("###\t\tMutator", mutator);
                InjectionConfigData injectionConfig = new InjectionConfigData(className, keyName, format, frequency);
    ...
    private static Map<String, ConnectionConfigData> getProfiles(Document doc, Map<String, ConnectionConfigData> connectionMap, String liveTarget, String enableEmailAlerts, String enableLogAlerts, String encryptionKey) throws IdMUnitException {
        ...
        for (int i = 0; i < profiles.size(); ++i) {
            ...
            for (int j = 0; j < connections.size(); ++j) {
                ...
                Map<String, String> substitutionMap = getSubstitutions(connection, encryptionKey);
                log("### \tData Injections:");
                ...
                if ("true".equalsIgnoreCase(enableEmailAlerts) || "true".equalsIgnoreCase(enableLogAlerts)) {
            ...
            //We don't need to process any more profiles, we found the live profile and grabbed the config data
    ...
    private static Map<String, Alert> getAlerts(Document doc, String enableEmailAlerts, String enableLogAlerts) throws IdMUnitException {
        if ((enableEmailAlerts == null || "false".equalsIgnoreCase(enableEmailAlerts)) && (enableLogAlerts == null || "false".equalsIgnoreCase(enableLogAlerts))) {
    ...
    public static Map<String, ConnectionConfigData> getConfigData(String configFileLocation, String encryptionKey) throws IdMUnitException {
        ...
        return getConfigData(doc, encryptionKey);
    }

    static Map<String, ConnectionConfigData> getConfigData(Document doc, String encryptionKey) throws IdMUnitException {
        ...
        if (liveTarget == null || liveTarget.length() < 1) {
            throw new IdMUnitException("Live target profile not specified. Ensure that the idmunit root node in idmunit-config.xml contains a live-profile specification. The target must be defined in the same xml file along with its corresponding connections.");
        }
    ...
    private static String getDecryptedChildText(Element elem, String name, String encryptionKey) throws IdMUnitException {
        Element child = elem.getFirstChildElement(name);
        if (child == null) {
            return null;
        }

        Attribute encryptedAttribute = child.getAttribute(XML_ATTRIBUTE_ENCRYPTED);
        boolean isEncrypted = encryptedAttribute == null ? false : Boolean.parseBoolean(encryptedAttribute.getValue());

        String value = child.getValue();

        if (isEncrypted || child.getQualifiedName().equals(XML_PASSWORD)) {
            if (encryptionKey == null) {
                throw new IdMUnitException("Unable to decrypt '" + name + "', no encryptionKey provided");
            }

            EncTool encryptionManager = new EncTool(encryptionKey);
            value = encryptionManager.decryptCredentials(value);
        }
