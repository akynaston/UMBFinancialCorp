    ...
    public static InitialDirContext createLdapConnection(String server, String userDN, String password, String keystorePath, String trustedCertFile, boolean useTLS, boolean trustAll, Map<String, String> envOverride) throws IdMUnitException {
        ...
        for (Map.Entry<String, String> entry : env.entrySet()) {
            if (env.containsKey(entry.getKey())) {
                log.info(String.format("Overriding configured LDAP connection option '%s' containing value '%s' with value '%s'.", entry.getKey(), env.get(entry.getKey()), entry.getValue()));
            } else {
                log.info(String.format("Adding additional LDAP connection option '%s' with value '%s'.", entry.getKey(), entry.getValue()));
            }
